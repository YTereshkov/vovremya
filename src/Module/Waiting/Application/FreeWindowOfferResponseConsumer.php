...
final readonly class FreeWindowOfferResponseConsumer implements NormalizedWebhookEventConsumer
{
    public function __construct(private FreeWindowOfferService $offers)
    {
    }
    ...
    public function consume(NormalizedWebhookEvent $event): void
    {
        ...
        if (preg_match('/^free-window-offer-accept:([0-9a-f-]{36}):([A-Za-z0-9_-]{16,64})$/D', $action, $matches)) {
        ...
        if (preg_match('/^free-window-offer-decline:([0-9a-f-]{36}):([A-Za-z0-9_-]{16,64})$/D', $action, $matches)) {
            if (null !== $id = $this->id($matches[1])) {
