...
final readonly class PermanentPlaceOfferController
{
    public function __construct(private PermanentPlaceOfferService $offers, private CsrfTokenManagerInterface $csrf)
    {
    }

    #[Route('/api/permanent-places/{id}/offers', methods: ['POST'])]
    public function create(string $id, Request $request, #[CurrentUser] AdministratorAccount $actor): JsonResponse
    {
        try {
            $this->checkCsrf($request);
            $data = $this->body($request);
            if (!is_string($data['waitingListEntryId'] ?? null)) {
                throw new \InvalidArgumentException('Некорректный получатель предложения.');
            }
            $offer = $this->offers->create($actor, Ulid::fromString($id), Ulid::fromString($data['waitingListEntryId']), new \DateTimeImmutable('now', new \DateTimeZone('UTC')));
    ...
    #[Route('/api/permanent-place-offers/{id}', methods: ['DELETE'])]
    public function cancel(string $id, Request $request): JsonResponse
    {
        try {
            $this->checkCsrf($request);
            $this->offers->cancel(Ulid::fromString($id), new \DateTimeImmutable('now', new \DateTimeZone('UTC')));
    ...
    private function body(Request $request): array
    {
        ...
        if (!is_array($data) || array_is_list($data) || array_diff(array_keys($data), ['waitingListEntryId'])) {
