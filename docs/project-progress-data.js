window.PROJECT_PROGRESS = {
  ...
  batches: [
    ...
    { number: 21, title: 'Уведомления и контроль доставки', parts: [{ number: 46, title: 'Уведомления и контроль доставки', status: 'done', completedAt: '2026-09-14', result: 'Агрегированный центр уведомлений показывает capability-aware delivery status, бизнес-ответ отдельно и ручной retry ошибок.' }] },
    {
      number: 22,
  ...
  changes: [
    {
      date: '2026-09-15',
      items: [
        'Frontend переведён на утверждённую тёплую серо-каменную дизайн-систему без изменений структуры экранов и бизнес-логики.',
        'Добавлены canvas, surface, surface-raised, plum primary, lavender accent и семантические status tokens; прямые белые поверхности и холодные фоновые засветки удалены.',
        'Общие shell, навигация, кнопки, поля, карточки, dialog, вкладки и календарные сетки используют единые семантические роли.',
        'По обратной связи canvas осветлён до #F3F1ED; основной цвет действий заменён на сливовый #65527C, а зелёный сохранён для статусов успеха. Desktop sidebar показывает счётчик уведомлений с внутренними отступами. Логотип использует чернильный цвет и ведёт на существующую страницу «Сегодня».',
        'Мобильная нижняя навигация показывает «Сегодня», «Календарь», «Уведомления», «Ожидание» с теми же иконками, что desktop; «Клиенты» и остальные разделы доступны через панель «Ещё» справа. На ширине 386–450 px шрифт и боковые отступы пунктов слегка уменьшены; до 385 px подписи скрыты. Для ширины 320 px исправлены тесные заголовки, обрезание полей времени и горизонтальная прокрутка, включая Browser с классической вертикальной полосой прокрутки. Только «Уведомления» имеют числовой бейдж: dev показывает тестовое 3 на desktop и mobile, production — фактический счётчик API. Добавлены короткие анимации переходов и reduced-motion fallback.',
        'Карточка специалиста показывает основное повторяющееся расписание списком, понятное пустое состояние и отдельно объясняет разовые рабочие дни. Термины действий и dialog приведены к этой модели.',
        'Создание клиента требует основной канал: ребёнок создаётся с обязательными именем и телефоном родителя, взрослый — со своим телефоном; получатель определяется автоматически и проверяется API.',
        'Текущий локальный demo-кабинет заполнен для ручного тестирования: график существующего специалиста, 5 клиентов, занятия распределены по 7 календарным неделям с 25 августа по 9 октября, для прошлых занятий заданы разные результаты и подтверждения, сохранены будущее свободное окно, 2 записи ожидания и 3 типа уведомлений.',
        'Отображаемые полные даты унифицированы в формате ДД.ММ.ГГГГ; дата и время выводятся как ДД.ММ.ГГГГ · ЧЧ:ММ. Нативные поля выбора даты сохраняют обязательный для браузера ISO-формат значения.',
        'Карточки и формы используют полную ширину родительского контейнера; дублирующая строка доступных каналов в настройках удалена. Поля повторного напоминания показываются только при включённой опции, смысловые группы получили доступные подсказки по hover и keyboard focus.',
        'Предпросмотр всех шаблонов сообщений подставляет демонстрационные дату, время, услугу, клиента и контакт вместо фигурных переменных. Подписи трёх кнопок подтверждения редактируются, сохраняются на уровне организации и используются в реальных исходящих сообщениях; приоритет отдельного шаблона услуги объяснён прямо в UI.',
        'Повторное напоминание отправляется не только при ожидании ответа, но и после подтверждения посещения; отменённые занятия и ответ «не сможем» исключены. Доступные подсказки добавлены к неоднозначным группам настроек услуги, отсутствия клиента и листа ожидания без перегрузки каждого поля.',
        'Typecheck, production build, calendar, catalog/clients и appointment warnings/conflicts E2E прошли на mobile и desktop; ключевые экраны проверены в Browser без overflow и console errors.',
      ],
    },
    {
      date: '2026-09-15',
    ...
    },
    {
      ...
      items: [
        ...
        'Добавлены per-admin read marker, tenant-scoped отчёт массовой отмены и ручной retry через durable outbox.',
  ...
  decisions: [
    { date: '2026-09-15', title: 'Цвет задаётся семантической ролью', text: 'Canvas, обычная поверхность и raised-поверхность разделены токенами; sage закреплён за действиями, lavender — за выбором и активной навигацией, status colors — за бизнес-состояниями. Прямые цвета допустимы только в объявлении токенов или с отдельным обоснованием.' },
