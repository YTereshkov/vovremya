...
export function SpecialistPage() {
  ...
  return <WorkforceFrame title="Специалист" back="/specialists" action={<button aria-label="Редактировать специалиста" className="p-2 text-primary" onClick={() => { update.reset(); setEditing(true) }}><Pencil className="size-5" /></button>}>
    <div className="space-y-4">
      ...
      <section className={surfaceClass}><h2 className="text-lg font-semibold">Рабочая неделя</h2>
        <div className="my-4 grid grid-cols-7 gap-2">{s.weeklyHours.map((d) => <span key={d.weekday} className={`rounded-lg border py-2 text-center text-sm ${d.enabled ? 'border-success text-success' : 'border-primary/20 text-muted'}`}>{shortDays[d.weekday - 1]}</span>)}</div>
        <div className="space-y-2 border-t border-border pt-4">{s.weeklyHours.filter((d) => d.enabled && d.work).map((d) => <p key={d.weekday}>{shortDays[d.weekday - 1]} · {d.work!.start}–{d.work!.end}{d.lunch ? <span className="ml-2 text-sm text-muted">Обед {d.lunch.start}–{d.lunch.end}</span> : null}</p>)}</div>
...
function AdditionalDayForm({ specialistId, initial, close }: { specialistId: string; initial?: AdditionalDay; close: () => void }) {
  ...
  return <Modal title={initial ? 'Дополнительный рабочий день' : 'Добавить рабочий день'} close={() => !change.isPending && close()}>
