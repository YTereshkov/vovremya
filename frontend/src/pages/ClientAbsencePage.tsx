...
export function ClientAbsencePage() {
  ...
  return <ResourceFrame title="Клиент отсутствует" back={`/clients/${id}`}>
    <form className="space-y-5" onSubmit={submit}>
      ...
      <fieldset className="space-y-3"><legend className="mb-2"><span className="flex items-center gap-2">Что сделать с занятиями<InfoHint label="Занятия на время отсутствия">Можно сохранить регулярное место и временно предложить освободившиеся окна другим клиентам либо полностью завершить регулярное расписание.</InfoHint></span></legend>
        <label className={`block rounded-2xl border p-5 ${mode === 'KEEP_PERMANENT_PLACE' ? 'border-accent bg-accent-soft' : 'border-border bg-surface'}`}><span className="flex items-start gap-3"><input checked={mode === 'KEEP_PERMANENT_PLACE'} className="mt-1 size-5 accent-accent" name="absence-mode" onChange={() => setMode('KEEP_PERMANENT_PLACE')} type="radio" /><span><strong className="block">Сохранить постоянное место</strong><span className="mt-1 block text-sm text-muted">После отсутствия регулярное расписание продолжится.</span></span></span>{mode === 'KEEP_PERMANENT_PLACE' ? <span className="mt-4 flex items-start gap-3 border-t border-accent/20 pt-4"><input checked={createFreeWindows} className="mt-1 size-5 accent-accent" onChange={(event) => setCreateFreeWindows(event.target.checked)} type="checkbox" /><span><strong className="block">Создать разовые свободные окна</strong><span className="text-sm text-muted">Интервалы можно будет предложить другим клиентам.</span></span></span> : null}</label>
        <label className={`flex items-start gap-3 rounded-2xl border p-5 ${mode === 'RELEASE_PERMANENT_PLACE' ? 'border-accent bg-accent-soft' : 'border-border bg-surface'}`}><input checked={mode === 'RELEASE_PERMANENT_PLACE'} className="mt-1 size-5 accent-accent" name="absence-mode" onChange={() => setMode('RELEASE_PERMANENT_PLACE')} type="radio" /><span><strong className="block">Освободить постоянное место</strong><span className="mt-1 block text-sm text-muted">Регулярное расписание клиента завершится.</span></span></label>
      </fieldset>
      <label className={`${resourceSurfaceClass} flex items-center justify-between gap-3`}><span>Уведомить клиента</span><input checked={notifyClient} className="size-6 accent-accent" onChange={(event) => setNotifyClient(event.target.checked)} type="checkbox" /></label>
