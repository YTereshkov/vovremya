...
export function WaitingListFormPage() {
  ...
  return <ResourceFrame back={`/clients/${id}`} title="Настроить ожидание">
    <form className="space-y-4" onSubmit={submit}>
      ...
      <section className={resourceSurfaceClass}><div className="mb-2 flex items-center gap-2 text-sm text-muted">Необходимая частота занятий<InfoHint label="Частота занятий">Система учитывает это число при подборе постоянных мест и показывает, сколько занятий в неделю клиенту ещё требуется.</InfoHint></div><select aria-label="Необходимая частота занятий" className={resourceFieldClass} value={frequency} onChange={(event) => setFrequency(Number(event.target.value))}>{[1, 2, 3, 4, 5, 6, 7].map((value) => <option key={value} value={value}>{value} {value === 1 ? 'раз' : value < 5 ? 'раза' : 'раз'} в неделю</option>)}</select></section>

      <section className={resourceSurfaceClass}>
        <div className="flex items-center gap-2"><h2 className="font-semibold">Подходящие дни и время</h2><InfoHint label="Доступность клиента">По этим интервалам система подбирает разовые окна и постоянные места. Пустое время «до» означает отсутствие верхней границы.</InfoHint></div>
      ...
      <section className={`${resourceSurfaceClass} flex items-center gap-2`}><label className="flex flex-1 items-center justify-between gap-4"><span>Готов приходить на разовые свободные окна</span><input className="size-6 accent-accent" checked={readyForOneOff} onChange={(event) => setReadyForOneOff(event.target.checked)} type="checkbox" /></label><InfoHint label="Разовые окна">Если включено, клиента можно предлагать на отдельные освободившиеся занятия без изменения его постоянного расписания.</InfoHint></section>
