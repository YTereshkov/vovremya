...
export function CalendarPage() {
  ...
  return <div className={cn('min-h-screen', fullscreen ? 'fixed inset-0 z-50 overflow-auto bg-[#f8faf8]' : null)}>
    <div className={cn('mx-auto w-full px-4 pb-28 pt-[calc(env(safe-area-inset-top)+20px)] sm:px-7 lg:px-8 lg:py-8', fullscreen ? 'max-w-none' : 'max-w-[1500px]')}>
      ...
      {!connected && !offline.isPending && !offline.error && !snapshot ? <p className="mt-6 rounded-lg border border-border bg-white/75 p-5 text-muted">Копия расписания недоступна. Подключитесь к интернету, чтобы синхронизировать данные.</p> : null}
      {!connected && snapshot && !withinSnapshot ? <p className="mt-6 rounded-lg border border-border bg-white/75 p-5 text-muted">За выбранный период нет сохранённых данных. Выберите дату из доступного диапазона.</p> : null}
