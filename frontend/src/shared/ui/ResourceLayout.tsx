...
export const resourceFieldClass = 'h-11 min-w-0 w-full rounded-lg border border-border bg-surface-raised px-3 outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 disabled:bg-surface disabled:text-muted'
export const resourceSurfaceClass = 'rounded-lg border border-border bg-surface p-5 shadow-surface'
...
export function ResourceModal({ title, close, children }: { title: string; close: () => void; children: ReactNode }) {
  ...
  return <dialog ref={ref} onCancel={close} aria-label={title} className="fixed inset-0 m-auto max-h-[90dvh] w-[calc(100%-32px)] max-w-lg overflow-auto rounded-lg border border-border bg-surface-raised p-5 text-ink shadow-surface backdrop:bg-ink/40">
